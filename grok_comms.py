...
Fleet Networks (v7):
  ...
  GrokComms.set_active_fleet()   — runtime fleet overlay; never mutates coord file
  GrokComms.clear_active_fleet() — return to personal coordinate
  GrokComms.poll_fleet_local()   — scan tight window around fleet V coordinate
...
Fleet Networks (v7):
  ...
  Fleet coordinate = temporary overlay stored only in GrokComms._active_fleet.
  poll_fleet_local(radius) scans a ±radius window around the fleet V.
...
class FleetRegistry:
    ...
    Fleet overlay is runtime-only, held in GrokComms._active_fleet.
    ...
    def register(
        ...
        self._save()
        print(f"  ✅ Fleet registered: {name}  V={str(v)[:20]}...  R={str(r)[:20]}...")
        return record
    ...
    def print_table(self):
        ...
        print(f"  {border}")
        print(f"  {'NAME':<22}  {'V (preview)':>22}  {'RADIUS':>8}  NOTES")
        print(f"  {'─'*22}  {'─'*22}  {'─'*8}  ─────")
        for name, rec in sorted(fleets.items()):
            v_p     = str(rec["v"])[:20] + "..."
            notes_p = (rec.get("notes") or "")[:24]
            print(f"  {name:<22}  {v_p:>23}  {rec.get('radius', '?'):>8}  {notes_p}")
        print(f"  {border}\n")
...
def range_padder(msg_json_path: str, coord_file: str = COORD_FILE) -> str:
    ...
    # PATCH v4: raise instead of silent fallback
    raise RuntimeError(
...
def send_outbox(coord_file: str = COORD_FILE):
    ...
    for fname in outbox_files:
        ...
        # PATCH v4: catch RuntimeError from range_padder and route to errors/
        try:
...
# ===========================================================================
# 12. GrokComms  — unified object-oriented interface  (v7: Fleet Networks)
# ===========================================================================
...
class GrokComms:
    ...
    v7 — Fleet Networks:
      - _active_fleet: runtime-only fleet overlay dict (never persisted to coord file)
      - _fleet_lock: RLock protecting _active_fleet reads/writes
      - fleet_registry: FleetRegistry instance for persistent fleet records
      - set_active_fleet(name, v, r, radius): overlay fleet coord at runtime
      - clear_active_fleet(): drop back to personal coord
      - get_active_fleet(): return current fleet dict or None
      - poll_fleet_local(): scan local fleet traffic window
      - poll_range(low, high): raw range scan used by daemon local-traffic hook

    Personal coordinate identity rule:
      coordinatefile.json is NEVER modified by any fleet operation.
      Fleet state lives only in self._active_fleet (memory) and
      fleet_registry.json (name→V,R lookup).

    Usage:
        gc = GrokComms("coordinatefile.json")
        gc.address_book.add_contact("Mom", some_coord, notes="Family channel")
        gc.send_realtime("bob", "Hello Bob")
        gc.poll_realtime()
        gc.poll()                     # unified temporal + beacon poll
        gc.set_active_fleet("Alpha", v=123456789, r=100000)
        gc.poll_fleet_local()         # scan fleet frequency
        gc.clear_active_fleet()
        print(gc.my_id)
    """
    ...
    def __init__(self, coord_file: str = COORD_FILE):
        ...
        self._active_fleet  = None                # v7: runtime fleet overlay
    ...
    def set_active_fleet(
        ...
        """
        Set the runtime fleet overlay.

        Personal coordinatefile.json is NEVER touched.
        Stores fleet (name, v, r, radius) in memory only.
        Also registers the fleet in the persistent FleetRegistry.

        Parameters
        ----------
        name   : fleet name / callsign
        ...
        with self._fleet_lock:
            ...
            # Persist to fleet registry so other subsystems can look it up
            self.fleet_registry.register(name, v, r, radius)
            print(f"  ⬡  [Fleet] Joined '{name}'  V={str(v)[:20]}...  R={str(r)[:20]}...")
            print(f"  ⬡  [Fleet] Personal coord unchanged — overlay active.")

    def clear_active_fleet(self):
        """
        Drop the fleet overlay and return to personal coordinate.
        Personal coordinatefile.json is unaffected.
        """
        with self._fleet_lock:
            if self._active_fleet:
                name = self._active_fleet.get("name", "?")
                print(f"  ⬡  [Fleet] Left '{name}' — returned to personal coord.")
                self._active_fleet = None
            else:
                print("  ⬡  [Fleet] No active fleet — already on personal coord.")

    def get_active_fleet(self) -> dict | None:
        """Return current fleet overlay dict, or None if not in a fleet."""
        with self._fleet_lock:
            return dict(self._active_fleet) if self._active_fleet else None
    ...
    def poll_fleet_local(
        ...
        """
        Scan the local traffic window around the active fleet's V coordinate.
        ...
        Parameters
        ----------
        radius     : override the fleet's stored radius (optional)
        num_probes : number of coordinate probes in the scan
        msg_length : bytes to decode per probe
        """
        with self._fleet_lock:
    ...
    def poll_range(self, low: int, high: int, msg_length: int = FLEET_LOCAL_MSG_LEN) -> list:
        ...
        Used by the daemon's local-traffic hook; also callable directly.
