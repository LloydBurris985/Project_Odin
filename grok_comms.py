...
=========================================
Modules:                                          coordinate_generator   — generate primary V/R coordinate, save to coordinatefile.json
  polling_range_finder   — sample random messages, compute RMS high/low, append to coord file     polling                — decode chart space in range, extract temporal headers, filter by To Date
  range_padder           — pad a composed message so its coordinate lands inside polling range    temporal_node          — interactive CLI: compose / inbox / received / sent / outbox / polling  admin_menu             — top-level menu wiring all modules                                    
...
def polling_range_finder(
    ...
    """
    Sample `num_samples` random messages of `message_length` bytes,
    encode each starting from V/R in coord_file, collect final coordinates.
    Compute RMS (root-mean-square) spread; set high/low polling range.
    Appends results back into coord_file.
    """
    print(f"\n[RangeFinder] message_length={message_length}  samples={num_samples}")
    coord = _load_json(coord_file)
    cg_base = _cg_from_coord(coord)
    base_V  = cg_base.hm.to_int(cg_base.Vs[0])

    samples = []
    tmp_dir = tempfile.mkdtemp(prefix="grok_range_")
    try:
        for i in range(num_samples):
            rand_msg = os.urandom(message_length)
            # fresh CG each sample, starting at the same V/R
            cg = _cg_from_coord(coord)
            for j in range(len(rand_msg) - 1, -1, -1):
                cg._encode_step(rand_msg[j], 0)
            final_V = cg.hm.to_int(cg.Vs[0])
            samples.append(final_V)
            if (i + 1) % 5 == 0:
                print(f"  [{i+1}/{num_samples}] sample coord: {str(final_V)[:20]}...")
    finally:
    ...
    polling_low  = int(mean - rms)
    polling_high = int(mean + rms)

    print(f"\n[RangeFinder] Mean        : {str(int(mean))[:30]}")
    print(f"[RangeFinder] Std Dev     : {std_dev:,.0f}")
    print(f"[RangeFinder] RMS spread  : {rms:,.0f}")
    ...
    coord["message_length"] = message_length
    coord["range_rms"]      = str(int(rms))
...
""")
            ...
            try:
                ...
                samples = 30
            polling_range_finder(msg_len, samples, coord_file)
...
def admin_menu():
    ...
    while True:
        ...
        if choice == "0":
            ...
            try:
                msg_len = int(input("  Message length bytes (Enter for 64): ").strip() or 64)
            except ValueError:
                msg_len = 64
            try:
                ...
                samples = 30
            polling_range_finder(msg_len, samples, coord_f)
