...
def service(runway):
    rw = RUNWAYS[runway]
    log(f"SERVICE {runway} - polling send + decoding EVERY coord in decoding section")
    for c in load_commands():
        if c.startswith("send") and runway in c:
            _, _, fpath = c.split(maxsplit=2)
            fpath = os.path.abspath(fpath)
            if os.path.exists(fpath):
                log(f"ENCODING at coding coord {rw['start']} (fixed size)")
                try:
        ...
        if f.endswith(".lattice") and runway in f:
            ...
            if os.path.exists(cf):
                ...
                if coord.get("start_p", 0) >= rw["mid"]:
                    log(f"DECODING coord {coord['start_p']} (full AB cross to sent territory)")
                    try:
                        ...
                        if os.path.exists(dec):
                            ...
                            log(f"DELIVERED fixed 1024B {msg_file}")
