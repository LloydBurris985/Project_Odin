...
if not os.path.exists(CONFIG):
    RUNWAYS = {
        "Runway_42": {"start_p": 500000, "length": 2000000, "sent": 0, "received": 0, "ratio": 100.0, "pass": 0, "app": None},
        "TEA": {"start_p": 1000000, "length": 5000000, "sent": 0, "received": 0, "ratio": 100.0, "pass": 0, "app": None},
        "Public": {"start_p": 700000, "length": 5000000, "sent": 0, "received": 0, "ratio": 100.0, "pass": 0, "app": None}
    }
...
def poll():
    for name, rw in RUNWAYS.items():
        ...
        if os.path.exists(QUEUE_FILE):
            ...
            for line in queue:
                if name in line:
                    ...
                    if os.path.exists(lattice):
                        ...
                        rw["ratio"] = ratio
                        log(f"✅ DECODED & RECOVERED on Pass {rw['pass']} | Ratio {ratio}%")
            ...
            log(f"Pass {rw['pass']} — scanning temporal stream")
    ...
    if os.path.exists(CMD_FILE):
        ...
        if cmd.startswith("send"):
            ...
            if runway in RUNWAYS and os.path.exists(file):
                ...
                log(f"✅ QUEUED & SENT on {runway}")
        elif cmd.startswith("register"):
            parts = cmd.split()
            runway = parts[1]
            app = parts[2] if len(parts) > 2 else "default"
            if runway in RUNWAYS:
                RUNWAYS[runway]["app"] = app
                log(f"✅ App {app} registered to {runway}")
...
def dashboard():
    while True:
        clear()
        print("╔════════════════════════════════════════════════════════════╗")
        print("║     STARSHIP ODIN — AIRPORT OPS v10 (DYNAMIC LIVE)         ║")
        print("║               Time Enforcement Agency (TEA)                ║")
        print("╚════════════════════════════════════════════════════════════╝\n")
        for name, rw in RUNWAYS.items():
            print(f"🛫 {name:12} Start: {rw['start_p']}  Len: {rw['length']:,}  Ratio: {rw['ratio']}%  Pass: {rw['pass']}  App: {rw.get('app','none')}")
...
if __name__ == "__main__":
    log("=== AIRPORT v10 DYNAMIC LIVE STARTED ===")
    dashboard()
