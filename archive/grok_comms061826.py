...
def register_beacon(
    ...
    Args:
        name       : human-readable beacon name, e.g. "OdinNet-Public-1"
        coordinate : the bare integer coordinate of this beacon
        msg_length : expected message length at this beacon
        notes      : optional description

    Returns:
        The beacon record dict.
    """
...
def polling_range_finder(
    ...
    Uses integer-only arithmetic to avoid OverflowError on large coordinates.
    """
    ...
    # Integer-only statistics — avoids OverflowError on large Burris integers
    mean    = _int_mean(samples)
...
def calculate_tight_polling_range(
    ...
    Saves rt_polling_low / rt_polling_high to coord_file (separate from
    the temporal node's polling_low / polling_high keys).
    Uses integer-only arithmetic to avoid OverflowError on large coordinates.
    """
    coord = _load_json(coord_file)
    ...
    # ±0.5σ  →  mean - std_dev//2  ..  mean + std_dev//2
    low  = mean - std_dev // 2
    high = mean + std_dev // 2
    if low >= high:
...
def send_realtime(
    ...
    Args:
        to         : recipient node id (or alias — resolved via address book if int fails)
        message    : plaintext message string
        reply_to   : msg_id of the message being replied to (optional)
        coord_file : path to coordinate JSON
        use_morse  : encode message as Morse before coordinate encoding
        my_id      : sender node id (auto-loaded from store if None)
...
def poll_realtime(
    ...
    Pass 1: store scan (reliable, deterministic).
    Pass 2: coordinate-space probe (best-effort window health check).
    """
    store = _load_rt_store()
    if my_id is None:
...
# ===========================================================================
# 12. GrokComms  — unified object-oriented interface
# ===========================================================================

class GrokComms:
    """
    Unified GrokComms object.  Holds a single coord_file reference and
    exposes all temporal + realtime + address book + beacon methods.
    ...
    Usage:
        ...
        print(gc.my_id)        # or gc.get_my_id()
    """
    ...
    def __init__(self, coord_file: str = COORD_FILE):
        ...
        store             = _load_rt_store()
        # Use 12-char UUID prefix for better uniqueness across multi-node setups
    ...
    def get_my_id(self) -> str:
        """Return this node's unique ID string."""
        return self.my_id
    ...
    def poll(self, use_beacons: bool = True) -> list[dict]:
        ...
        Args:
            use_beacons : if True (default), poll all registered beacons in
                          addition to the coordinate-space window.

        Returns:
            Combined list of received message records.
        """
        coord = _load_json(self.coord_file)

        # Guard: polling range must be set
        ...
        # Beacon poll runs independently of whether temporal range exists
        if use_beacons:
            beacon_msgs = _poll_beacons(coord)
            # beacon messages are already appended inside polling() when called
            # via that path, but here we only add them if we skipped polling()
    ...
    def send_realtime(
        ...
        # Allow alias lookup: if 'to' is in the address book, resolve it
    ...
    while True:
        ...
        if cmd in ("exit", "quit"):
            ...
            if len(parts) < 3:
                ...
                continue
            # Resolve alias from address book
            to_raw = parts[1]
...
# ── Temporal Comms CLI (formerly Temporal Node) ───────────────────────────

def temporal_comms(coord_file: str = TNODE_COORD):
    """
    Interactive Temporal Communications Node CLI.
    Polls both coordinate space and registered beacons during 'polling'.
    """
    _ensure_dirs()
...
# ===========================================================================
# 14. admin_menu  — top-level entry point
# ===========================================================================
