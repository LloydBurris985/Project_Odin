...
class LatticeDrive:
    ...
    def _encode_bytes_up(self, cg: ChartGenerator, data: bytes, u: int = 0) -> int:
        ...
        # Use to_int which reads all D limbs (D may have auto-expanded during encode)
        return hm.to_int(cg.Vs[u])

    def _decode_bytes_up(self, cg: ChartGenerator, V: int, length: int, u: int = 0) -> bytes:
        hm = cg.hm
        # Ensure hm.D is large enough to represent V without truncation.
        # V may have been produced with an auto-expanded D; grow to match.
        needed = 0
        v_tmp  = V
        while v_tmp > 0:
            needed += 1
...
class LatticeFS:
    ...
    def set_encryption_key(self, passphrase: str):
        """
        Enable (or rotate) superblock encryption.
        Immediately re-flushes the superblock with the new key.
    ...
    def _flush_index(self):
        ...
        if len(payload) > self._drive.sector_size:
            # Auto-grow: no hard limits. Expand sector_size to fit.
        ...
        # Ensure sector 0's stored byte_length always matches the full payload
        # so read() returns every byte (not just the original smaller size).
    ...
    def _load_index(self):
        ...
        # Strip sector zero-padding from both plaintext and encrypted blobs.
        # - Plaintext JSON:       null bytes are only padding, safe to strip.
        # - Encrypted base64:     base64 chars are always ASCII (>= 0x20), never 0x00.
        #   The ciphertext itself is stored as base64, so trailing 0x00 is always padding.
    ...
    def write_file(self, filename: str, data: bytes) -> dict:
        ...
        # In-place overwrite if new data fits in existing allocation
    ...
    def compact(self) -> "LatticeFS":
        ...
        self._drive.format()
        self._index     = {}
        self._url_index = self._url_index   # preserve URL registry across compact
        self._next_free = 1
    ...
    def register_url(self, url_path: str, coordinate: int, metadata: dict = None) -> dict:
        """
        Register a burris:// URL → coordinate mapping.

        Args:
            url_path   : URL path string, e.g. "burris://my-site.com/about"
                         or just "/about" — scheme optional, stored as given.
            coordinate : large integer coordinate this URL resolves to.
            metadata   : optional dict with keys like size, hash, mime_type,
                         version, to_date, description.
    ...
    def resolve_url(self, url_path: str) -> dict | None:
        """
        Resolve a burris:// URL to its coordinate + metadata.

        Returns the registry entry dict, or None if not found.
        The 'coordinate' value is returned as an int.
        """
...
if __name__ == "__main__":
    ...
    # Encryption round-trip (save/reload drive, re-mount FS)
