...
Decode UP (direct inverse):
    num   = V + R * (BASE - 1)      ← uses R, not a constant
    V_old = num // BASE
    byte  = num %  BASE
...
class ChartGenerator:
    ...
    def fold_r(self, universe: int = 0, verbose: bool = False) -> bool:
        ...
        This keeps encoding numerically stable over very long sequences by
        preventing |V - R| from growing unboundedly relative to R.
    ...
    def _decode_step(self, u: int = 0) -> int:
        """
        CORRECT inverse of _encode_step.

        Encode:  V_new = V + (V - R) * (BASE - 1) + byte
                       = V * BASE - R * (BASE - 1) + byte

        Inverse: num   = V + R * (BASE - 1)
                 V_old = num // BASE
                 byte  = num %  BASE

        R must be the SAME value used during encoding of that step.
        """
        hm   = self.hm
        BASE = self.chart_base
        V, R = self.Vs[u], self.Rs[u]

        r_offset       = hm.mul_scalar(R, BASE - 1)
        num            = hm.add(V, r_offset)
...
╚══════════════════════════════════════════════════════════════╝"""
    ...
    def decode_file(self, input_json_path: str, output_path: str):
        ...
        self.Rs[u]      = self.hm.deserialize(state["R"])  # R MUST load before V
...
class LatticeFS:
    ...
    Features:
      ...
      - burris:// URL registry (register_url / resolve_url / unregister_url)
...
if __name__ == "__main__":
    ...
    cg_fold.FOLD_DRIFT_THRESHOLD = 0
    for b in range(50):
    ...
    fs.unregister_url("burris://odinnet.io/about")
    assert fs.resolve_url("burris://odinnet.io/about") is None
    print("✅  unregister_url PASSED")
