...
# ===========================================================================
# EDUCATION SEED  (K-12 Starter Pack for LatticeFS)
# Replaces the minimal _seed_lattice_fs above when called with edu=True.
# The daemon calls this automatically on first boot.
# ===========================================================================
...
### Try it mentally
Imagine counting steps on a number line that is
10^150 (that is 1 followed by 150 zeros) steps long.
BNS messages live somewhere on that line.
""",
...
### Decoding reverses it exactly
    ...
    num   = V + BASE - 1
    V_old = num ÷ BASE        (integer division)
    byte  = num mod BASE      (remainder)

### Why this works
Each encode step is a one-to-one function.
Every V has exactly one V_old and one byte.
That means **no information is lost** — perfect fidelity.

### Grade level note
- Grades 1–3 : think of it as a secret counting game
- Grades 4–6 : it is arithmetic with very big numbers
- Grades 7–9 : it is a bijective base-conversion function
- Grades 10+ : it is arithmetic coding without a probability model
""",
...
Because coordinates are large, you cannot check every one.
Instead, BNS picks a range where *real messages tend to land*
based on statistics — the average (mean) and spread (std dev)
of many sample encodings.

### The two windows
| Window    | Use         | Width           |
|-----------|-------------|-----------------|
| Temporal  | slow mail   | mean + 3.5σ high|
| Realtime  | fast chat   | tight ±0.5σ     |

### Analogy
Imagine you always park your car within 5 minutes walk
of your office. A friend searching for your car would
start looking in that radius — not across the whole city.
The polling window is that search radius.

### What σ means
σ (sigma) is the standard deviation — a measure of how
spread out the sample coordinates are.
A smaller σ means messages cluster tightly together.
""",
...
Ships (nodes) look for the lighthouse to know where they are
and to find messages left for the public.

### Beacon facts
- Anyone can register a beacon
- Beacons are polled automatically during every scan
- Each beacon has a **reputation score** (0–100)
- Bad behaviour (decode failures, hash mismatches) lowers the score
- A score below the DEFCON threshold causes expulsion
...
At DEFCON 7 and above, only server-run beacons are trusted.
""",
...
Every node needs a **coordinate file** (coordinatefile.json).
This is your identity in BNS space.

    python grok_comms.py
    > 1  (coordinate_generator)
    > Enter passphrase: my-secret-phrase

Keep your passphrase safe — it is the key to your identity.

### Step 2 — Set your polling range
    > 2  (polling_range_finder)

This samples 30 random messages and computes where
your messages will land in coordinate space.
...
def _seed_lattice_fs_with_education(fs: "LatticeFS"):
    """
    Seed a fresh LatticeFS with:
      - dashboard (index.html)
      - readme.txt
      - K-12 education starter pack (edu/ folder, 6 lessons)
    """
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if os.path.exists(DASHBOARD_HTML):
        ...
        dashboard_bytes = (
            f"<html><body><h1>OdinNet</h1><p>Seeded {now}</p>"
...
""".encode("utf-8")
    ...
    print(f"  LatticeFS  : seeded {lesson_count} education lesson(s) in edu/")
