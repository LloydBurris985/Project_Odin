"""
OdinNet Daemon  v7.4 — Pythagorean Rolling Wave Engine
=======================================================
Background service for OdinNet.

Changes from v7.3:
  - NEW: FoldingChartGenerator (stateless Pythagorean Rolling Wave) is now
         the default coordinate engine for all LatticeFS and BNS operations.
  - NEW: FoldingLatticeDrive is now the default block device for LatticeFS.
         Drive images saved as odinnet_folding_drive.json.
  - NEW: Legacy ChartGenerator / LatticeDrive retained as named fallback
         (--legacy-lattice CLI flag or if folding modules are absent).
  - NEW: _try_load_folding_lattice_fs() — bootstraps FoldingLatticeDrive-
         backed LatticeFS with digit-length guard on all sector I/O.
  - NEW: /api/engine endpoint now reports lattice_engine field.
  - KEPT: All v7.3 stateless comms, fleet, BBS, privacy, and beacon
          endpoints unchanged.
  - KEPT: GrokComms / StatelessCommsNode bridge architecture unchanged.
  - KEPT: Background pollers, anomaly detector, GUI bootstrap unchanged.
"""
...
if _fcg_mod is not None and _fld_mod is not None:
    ...
    print("  FoldingEngine : FoldingChartGenerator + FoldingLatticeDrive ACTIVE",
          flush=True)
else:
    ...
    print("  ⚠  folding_chart_generator / folding_lattice_drive not found — "
          "falling back to legacy ChartGenerator / LatticeDrive.", flush=True)

# ── Stateless comms — optional ────────────────────────────────
if _sc_mod is not None:
    ...
    print("  ⚠  stateless_comms not found — stateless engine disabled. "
          "Running in legacy mode.", flush=True)
...
# Lattice image paths — folding drive gets its own file so legacy image is untouched
...
# ─────────────────────────────────────────────────────────────
# LatticeFS bootstrap  — folding (new default) + legacy fallback
# ─────────────────────────────────────────────────────────────

def _seed_fs(fs: "LatticeFS"):
    """Seed a freshly created LatticeFS with dashboard and readme."""
...
def _try_load_folding_lattice_fs(passphrase: str = None) -> "LatticeFS | None":
    """
    Bootstrap a FoldingLatticeDrive-backed LatticeFS.
    Falls back gracefully to legacy if folding modules are absent.
    Digit-length guard is handled inside FoldingChartGenerator.encode_bytes /
    decode_bytes — no extra wrapping needed here.
    """
    if not _FOLDING_AVAILABLE:
        return _try_load_legacy_lattice_fs(passphrase)

    try:
        if os.path.exists(FOLDING_LATTICE_IMAGE_PATH):
            drive = FoldingLatticeDrive()
            drive.load(FOLDING_LATTICE_IMAGE_PATH)
            fs = LatticeFS(drive, passphrase=passphrase)
            print(f"  LatticeFS  : folding drive loaded ← {FOLDING_LATTICE_IMAGE_PATH}",
                  flush=True)
            return fs

        # Fresh creation
        drive = folding_lattice_drive_fn(
            sector_size  = 1024,
            n_sectors    = 256,
            scale_factor = 5000,
        )
        fs = LatticeFS(drive, passphrase=passphrase)
        _seed_fs(fs)
        drive.save(FOLDING_LATTICE_IMAGE_PATH)
        print(f"  LatticeFS  : folding drive created fresh → {FOLDING_LATTICE_IMAGE_PATH}",
              flush=True)
        return fs

    except Exception as e:
        print(f"  ⚠  FoldingLatticeDrive init failed: {e} — falling back to legacy.",
              flush=True)
        return _try_load_legacy_lattice_fs(passphrase)


def _try_load_legacy_lattice_fs(passphrase: str = None) -> "LatticeFS | None":
    """Legacy LatticeDrive-backed LatticeFS (unchanged from v7.3)."""
    try:
        if os.path.exists(LATTICE_IMAGE_PATH):
            drive = LatticeDrive()
            drive.load(LATTICE_IMAGE_PATH)
            fs = LatticeFS(drive, passphrase=passphrase)
            print(f"  LatticeFS  : legacy drive loaded ← {LATTICE_IMAGE_PATH}",
                  flush=True)
            return fs
        ...
        print(f"  LatticeFS  : legacy drive created fresh → {LATTICE_IMAGE_PATH}",
              flush=True)
        return fs
    except Exception as e:
        print(f"  ⚠  LatticeFS init failed: {e}", flush=True)
        return None
...
def _save_lattice(fs: "LatticeFS", folding: bool = True):
    path = FOLDING_LATTICE_IMAGE_PATH if folding else LATTICE_IMAGE_PATH
    try:
        fs._drive.save(path)
    except Exception:
        pass
...
def _ensure_coordinate(coord_file: str) -> bool:
    ...
    try:
        coordinate_generator(
            f"odinnet-{socket.gethostname()}",
            num_digits=150,
            output_file=coord_file,
        )
...
class DaemonContext:
    """
    Central shared-state container for the OdinNet daemon v7.4.

    Lattice engine selection
    ------------------------
    self._lattice_engine  — "folding" | "legacy"
      "folding"  : FoldingLatticeDrive + FoldingChartGenerator (new default)
      "legacy"   : LatticeDrive + ChartGenerator (fallback / --legacy-lattice)

    Comms engine selection (unchanged from v7.3)
    --------------------------------------------
    self.stateless_node  — StatelessCommsNode (primary)
    self.fallback_comms  — GrokComms          (legacy)
    self.active_comms    — alias to whichever is active
    """
    ...
    def __init__(
        ...
        lattice_engine:  str  = "folding",   # "folding" | "legacy"
    ):
        # ── Comms engine layer ────────────────────────────────────────────
        ...
        # ── Lattice engine layer ──────────────────────────────────────────
        self.fs             = fs
        self._lattice_engine = lattice_engine   # reported in /api/engine
    ...
    def get_status(self) -> dict:
        ...
        return {**ctx_status, **engine_status}
...
class OdinWebHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            if path in ("/status", "/api/status"):
                ...
                ctx  = self.daemon_ctx
                info = {
                    "comms_engine":   ctx._engine_name,
                    "lattice_engine": ctx._lattice_engine,
                    "stateless":      ctx.stateless_node is not None,
                    "folding":        _FOLDING_AVAILABLE,
                    "legacy_comms":   True,
                }
    ...
    def _serve_status_html_fallback(self):
        ...
        body = (
            ...
            f'<body><h1>⬡ OdinNet Node Panel v7.4</h1>'
            f'<p>ID: {d.get("node_id","?")}  |  '
            f'Comms: {d.get("engine","?")}  |  '
            f'Lattice: {d.get("lattice_engine","?")}  |  '
    ...
    def _handle_send(self):
        ...
        self._respond(200 if ok else 500, "application/json",
                      json.dumps({"ok": ok, **result}).encode())
...
def _run_web_server(ctx: DaemonContext, host: str, port: int):
    ...
    try:
        ...
        ctx.log(f"OdinWeb v7.4 running on port {port}  "
                f"comms={ctx._engine_name}  lattice={ctx._lattice_engine}")
...
def main():
    parser = argparse.ArgumentParser(description="OdinNet Engine v7.4")
    ...
    parser.add_argument("--legacy-lattice", action="store_true",
                        help="Force legacy LatticeDrive instead of FoldingLatticeDrive")
    ...
    # ── GrokComms (always needed for fleet/BBS) ───────────────────────────
    ...
    # ── LatticeFS — folding (default) or legacy (--legacy-lattice) ───────
    use_folding    = _FOLDING_AVAILABLE and not args.legacy_lattice
    lattice_engine = "folding" if use_folding else "legacy"

    if use_folding:
        fs = _try_load_folding_lattice_fs()
    else:
        fs = _try_load_legacy_lattice_fs()

    if fs:
        _update_dashboard_in_fs(fs)
        _save_lattice(fs, folding=use_folding)
    ...
    ctx = DaemonContext(
        ...
        lattice_engine = lattice_engine,
    )

    # ── Background threads ─────────────────────────────────────────────────
    threading.Thread(
        target=_legacy_background_poller, args=(ctx,),
        daemon=True, name="LegacyPoller",
    ).start()

    if stateless_node is not None:
        threading.Thread(
            target=_stateless_background_poller, args=(ctx,),
            daemon=True, name="StatelessPoller",
        ).start()

    threading.Thread(
        target=_anomaly_detector_loop, args=(ctx,),
        daemon=True, name="AnomalyDetector",
    ).start()
    ...
    print(f"\n{'='*58}")
    print(f"  ⬡  ODINNET NODE v7.4  —  PYTHAGOREAN ROLLING WAVE")
    print(f"{'='*58}")
    print(f"  Node ID        : {args.node_id}")
    print(f"  Passphrase     : {args.passphrase[:4]}****")
    print(f"  Comms engine   : {ctx._engine_name}")
    print(f"  Lattice engine : {lattice_engine}")
