#!/usr/bin/env python3
"""
OdinNet Daemon  v7 — Fleet Networks (Multiple Concurrent Fleets)
================================================================
...
class DaemonContext:
    ...
    def join_fleet(
        ...
        """
        Join a fleet overlay at the daemon level AND in GrokComms.
        ADDITIVE — multiple concurrent fleet memberships supported.
    ...
    def leave_fleet(self, name: str = None):
        ...
        Delegates to GrokComms.clear_active_fleet().
        """
        with self._fleet_lock:
    ...
    def leave_all_fleets(self):
        """Leave every active fleet."""
        self.leave_fleet(name=None)

    def get_current_fleets(self) -> dict:
        """Return a copy of all active fleet dicts keyed by name."""
        return self.comms.get_active_fleets()

    def get_all_local_traffic_ranges(self) -> list[tuple[str, int, int]]:
        """
        Return list of (name, low, high) for ALL active fleet local scans.
        Returns empty list if no fleets are active.
        """
    ...
    def status_dict(self) -> dict:
        ...
        active_fleets = self.get_current_fleets()   # v7: all active fleets
...
def _background_poller(ctx: DaemonContext):
    ctx.log("Temporal Node background poller started. (v7 — Multi-Fleet Networks)")
    ctx.refresh_beacons()
    ...
    while True:
        try:
            ...
            # ── Fleet local traffic — ALL active fleets (v7) ──────────────
...
class OdinWebHandler(http.server.BaseHTTPRequestHandler):
    ...
    GET  /status                        → JSON status (includes active_fleets)
...
<body>
  ...
  <pre>{chr(10).join(d['activity_tail'][-20:])}</pre>
    ...
    def _serve_fleet_status(self):
        """GET /api/fleet/status — ALL active overlays + personal coord."""
        ctx           = self.daemon_ctx
    ...
    def _handle_fleet_join(self):
        ...
        Body: {
          ...
          "v":      123456789,        ← required (or omit if in registry)
          "r":      100000,           ← required (or omit if in registry)
          "level":  "fleet",          ← optional: fleet|battlegroup|armada|grand_fleet
          "radius": 5000              ← optional: local scan radius (default 5000)
        }

        ADDITIVE — joins without leaving any currently active fleets.
        Personal coordinatefile.json is NEVER modified.
        If fleet is in fleet_registry.json, v/r/radius can be omitted.
        """
        ctx  = self.daemon_ctx
        body = self._read_json_body()
        if not body:
    ...
    def _handle_fleet_leave(self):
        ...
        Body: {"name": "Alpha Fleet"}   ← leave one fleet
        Body: {}                        ← leave ALL fleets

        Clears the named fleet overlay (or all overlays) and returns
        to personal coordinate polling.
        """
        ctx  = self.daemon_ctx
        body = self._read_json_body() or {}
...
def _print_startup_banner(
    ...
    print(f"\n{border}")
    print(f"  ODINNET DAEMON  v7  —  Multi-Fleet Networks  —  {now}")
    ...
    print(f"")
    print(f"  # Join a second fleet simultaneously:")
    print(f"  curl -X POST http://{lan_ip}:{port}/api/fleet/join \\")
    print(f'       -H "X-OdinNet-Token: {API_TOKEN}" \\')
    print(f'       -H "Content-Type: application/json" \\')
    print(f"       -d '{{\"name\":\"BravoFleet\",\"v\":987654321,\"r\":200000}}'")
    print(f"")
    print(f"  # Check all active fleets:")
    print(f"  curl http://{lan_ip}:{port}/api/fleet/status \\")
    print(f'       -H "X-OdinNet-Token: {API_TOKEN}"')
    print(f"")
    print(f"  # Leave one fleet:")
    print(f"  curl -X POST http://{lan_ip}:{port}/api/fleet/leave \\")
    print(f'       -H "X-OdinNet-Token: {API_TOKEN}" \\')
    print(f'       -d \'{{\"name\":\"AlphaFleet\"}}\'')
    print(f"")
    print(f"  # Leave ALL fleets:")
    print(f"  curl -X POST http://{lan_ip}:{port}/api/fleet/leave \\")
    print(f'       -H "X-OdinNet-Token: {API_TOKEN}"')
    print(f"  ─────────────────────────────────────────────────────────────\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="OdinNet Daemon v7 — Burris Numerical System Multi-Fleet Networks",
