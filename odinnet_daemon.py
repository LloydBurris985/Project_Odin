...
v12.4.0  (Phase 2A pipeline wiring — Step 5, Scotty/Claude)
                                              Changes from v12.3.0
------------------------------------------------
21. PHASE 2A PIPELINE WIRED IN — PollingManager now constructed with
    mission_profile="standard" (council-proposed preset, numeric values
    NOT yet ratified — see polling_manager.py module docstring item 8).
    Easy one-line change to "stealth"/"burst"/None once you've had a
    chance to react to it in practice.

    REAL GAP FOUND, NOT SILENTLY FIXED: prior to this version,
    PollingManager.perform_batch_poll()/run_forever() were never called
    anywhere in the daemon automatically — only BeaconPoller.sweep() was
    scheduled (via start_beacon_loop()). Real/temporal polling only ran
    on-demand through /api/stateless/poll, which calls
    ctx.stateless_node.poll() DIRECTLY and bypasses PollingManager (and
    therefore the entire Phase 2A pipeline) entirely. That endpoint is
    left unchanged here — manual polls still work exactly as before,
    explicit steps value and all.

    To make the pipeline actually run automatically, this version adds
    a background thread calling ctx.polling_manager.run_forever()
    (poll_interval=60s, release_interval=3600s), mirroring the existing
    start_beacon_loop() pattern. THIS IS A REAL BEHAVIOR CHANGE — nothing
    ran automated real/temporal polling before tonight. If you'd rather
    keep it fully on-demand for now, comment out the one
    self._polling_thread.start() block below; everything else in this
    diff (mission_profile wiring, version bump) is inert without it.
...
class DaemonContext:
    def __init__(self):
        ...
        self.polling_manager = PollingManager(
            ...
            # NEW (v12.4.0, Step 5): activates the Phase 2A pluggable polling
            # pipeline (TriangleWaveStrategy + DriftController + AmplitudeController).
            # "standard" is a proposed preset, not council-ratified — see
            # polling_manager.py module docstring item 8. Change to "stealth",
            # "burst", or None (legacy flat steps=100) with no other code changes.
        ...
        # NEW (v12.4.0, Step 5): automates real/temporal polling via
        # PollingManager.run_forever() — see changelog item 21 for why this
        # is a real behavior change, not just plumbing. Comment out this
        # block to keep real/temporal polling fully on-demand (via
        # /api/stateless/poll) instead of automatic.
        self._polling_thread = threading.Thread(
            target=self.polling_manager.run_forever,
            kwargs={"poll_interval": 60, "release_interval": 3600},
            daemon=True,
        )
