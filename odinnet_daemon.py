...
Changes from v4:
  - /api/fleet/jump endpoint: expel bad actors, broadcast new R, persist manifest
  - fleet jump result included in /status JSON (current_r, jump_count, last_jump)
  - "Temporal Node" label used throughout banner and logging
  - _seed_lattice_fs replaced by _seed_lattice_fs_with_education on first boot
  - All prior v4 patches retained (RLock, bearer token auth, security endpoints)
...
def _seed_lattice_fs_with_education(fs: "LatticeFS"):
    """
    Seed a fresh LatticeFS with dashboard + readme + K-12 education starter pack.
    Called only on first boot (when no drive image exists on disk).
    """
...
def _background_poller(ctx: DaemonContext):
    ctx.log("Temporal Node background poller started.")
    ctx.refresh_beacons()
...
class OdinWebHandler(http.server.BaseHTTPRequestHandler):
    """
    OdinWeb HTTP handler — v5.

    GET  /                        → dashboard from LatticeFS
    GET  /status                  → JSON status (includes fleet jump fields)
    ...
    POST /api/security/raise      → raise DEFCON                [token required]
    POST /api/security/lower      → lower DEFCON                [token required]
    ...
    POST /api/fleet/jump          → Fleet Jump: expel+broadcast [token required]  ← NEW v5
    GET  /api/fleet/manifest      → jump manifest JSON          [token required]  ← NEW v5
    """
...
<body>
  ...
  <pre>{chr(10).join(d['activity_tail'][-20:])}</pre>
    ...
    def _serve_fleet_manifest(self):
        """GET /api/fleet/manifest — return full jump manifest as JSON."""
        ctx = self.daemon_ctx
        if not ctx.security:
            self._respond(503, "application/json",
                          b'{"ok":false,"error":"Security module not available"}')
    ...
    # ── /api/fleet/jump  (NEW v5) ─────────────────────────────────────────

    def _handle_fleet_jump(self):
        """
        POST /api/fleet/jump
        Body: { "new_r": <int>, "reason": "<string>", "force": false }

        Sequence:
          1. Validate token (already done by do_POST gate)
          2. Validate body: new_r must be a positive integer
          3. Load current beacon list from disk
          4. Call security.fleet_jump() — expel bad actors, persist manifest,
             broadcast new R to trusted beacons
          5. Return full result JSON to dashboard
        """
        if not self._security_required():
            return
        ...
        try:
            # Load current beacon list so fleet_jump can expel + broadcast
            ...
            # Refresh beacon status in daemon context (expelled ones are now gone)
            ctx.refresh_beacons()
            self._respond(200, "application/json",
                          json.dumps(result, indent=2).encode())
        except ValueError as e:
            # DEFCON gate or duplicate-R rejection
...
def _print_startup_banner(
    ...
    print(f"  Endpoints   : /api/fleet/jump  /api/fleet/manifest  (NEW v5)")
    print(f"{border}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="OdinNet Daemon v5 — Burris Numerical System Temporal Node",
