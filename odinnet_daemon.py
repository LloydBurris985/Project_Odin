...
Changes from v5:
  - AUTO PORT RECOVERY: on OSError 98 (address in use), kill old PID and retry
  - HOST default changed to 0.0.0.0 (required for phone browser access)
  - Temporal reply_id / reply_to support in compose + received log
  - Anomaly Detector: hourly status report composed 24h into the past with hash
  - Faster polling: POLL_INTERVAL_SEC reduced to 8s; realtime to 5s sub-cycle
  - /api/publish endpoint: register burris:// URLs in LatticeFS URL index
  - GET /api/publish/list: list all published URLs
  - BBS stub: Odin's Hall with "The Thing" room
    GET  /api/bbs/rooms          — list rooms
    GET  /api/bbs/room/<name>    — get posts in room
    POST /api/bbs/post           — post to room  [token required]
  - Privacy: message recipient field enforced; cross-recipient isolation note
  - User block/mute list in address book (stored in blocked.json)
  - All prior v5 patches retained
...
DEFAULT_HOST          = "0.0.0.0"          # ← FIXED: was 127.0.0.1
DEFAULT_PORT          = 8080
POLL_INTERVAL_SEC     = 8                  # ← FASTER: was 15s
POLL_RT_INTERVAL_SEC  = 5                  # ← NEW: realtime sub-cycle
POLL_ERROR_BACKOFF    = 20
ANOMALY_INTERVAL_SEC  = 3600              # hourly anomaly reports
...
# ─────────────────────────────────────────────────────────────
# Port conflict recovery  (NEW v6)
# ─────────────────────────────────────────────────────────────

def _free_port(port: int) -> bool:
    """
    If something is already bound to `port`, try to kill it.
    Returns True if port is now free.
    """
    pid_file = os.path.join(_DAEMON_DIR, f"odinnet_{port}.pid")

    # 1. Try our own PID file first
...
# ─────────────────────────────────────────────────────────────
# Block / Mute list  (NEW v6)
# ─────────────────────────────────────────────────────────────

def _load_blocked() -> dict:
    """Returns {"blocked": [...], "muted": [...]}"""
...
# ─────────────────────────────────────────────────────────────
# Anomaly Detector  (NEW v6)
# ─────────────────────────────────────────────────────────────

def _anomaly_hash(node_id: str, timestamp: str, status_snapshot: dict) -> str:
    """
    SHA-256 hash of node_id + timestamp + key status fields.
    Provides tamper-evident fingerprint for anomaly reports.
    """
    payload = json.dumps({
        "node_id":    node_id,
        "timestamp":  timestamp,
...
def _anomaly_detector_loop(ctx: "DaemonContext"):
    """
    Background thread: every hour compose a temporal status report
    dated 24 hours into the past, with a verification hash.
    """
    # Stagger first run by 60s so daemon is fully warmed up
    time.sleep(60)
...
class DaemonContext:
    ...
    def __init__(
        ...
        # BBS and published URL caches (loaded fresh from disk on each request)
    ...
    def add_temporal_received(self, records: list):
        """
        Add temporal messages to received log.
        Privacy: each record must have a 'recipient' field.
        Messages are stored with recipient tag for cross-visibility isolation.
        """
        for r in records:
    ...
    def get_messages_for_recipient(self, recipient: str, log: str = "temporal") -> list:
        """
        Privacy filter: only return messages where recipient matches,
        OR where recipient is empty (broadcast / legacy messages).
        This prevents cross-visibility on shared coordinates.
        """
        with self._lock:
...
# ─────────────────────────────────────────────────────────────
# Background poller thread  (v6: faster, interleaved RT)
# ─────────────────────────────────────────────────────────────

def _background_poller(ctx: DaemonContext):
    ctx.log("Temporal Node background poller started. (v6 — 8s cycle)")
    ctx.refresh_beacons()

    rt_counter = 0  # run realtime every N ticks

    while True:
        try:
            # ── Temporal poll (every tick) ───────────────────────────
            ...
            # ── Realtime poll (every tick, separate sub-interval) ────
...
class OdinWebHandler(http.server.BaseHTTPRequestHandler):
    ...
    POST /api/fleet/jump                → Fleet Jump                  [token]
    GET  /api/fleet/manifest            → jump manifest JSON          [token]

    POST /api/publish                   → register burris:// URL      [token]  NEW v6
    GET  /api/publish/list              → list published URLs                  NEW v6

    GET  /api/bbs/rooms                 → list BBS rooms                       NEW v6
    GET  /api/bbs/room/<name>           → get posts in room                    NEW v6
    POST /api/bbs/post                  → post to BBS room            [token]  NEW v6

    POST /api/privacy/block             → block a coordinate/alias    [token]  NEW v6
    POST /api/privacy/mute              → mute a coordinate/alias     [token]  NEW v6
    GET  /api/privacy/list              → list blocked/muted          [token]  NEW v6
    POST /api/privacy/unblock           → unblock                     [token]  NEW v6
    """
    ...
    def _serve_status_html_fallback(self):
        ...
        if sec.get("jump_count"):
            jump_line = f"<p>Fleet Jumps: {sec['jump_count']} | Current R: {sec.get('current_r', 1)}</p>"
...
<body>
  ...
  <pre>{chr(10).join(d['activity_tail'][-20:])}</pre>
    ...
    def _serve_bbs_room(self, room_key: str):
        ...
        if not room:
            self._respond(404, "application/json",
                          json.dumps({"ok": False, "error": f"Room '{room_key}' not found"}).encode())
            return
        # Return last 50 posts newest-first
    ...
    # ── /api/compose  (v6: reply_id / reply_to support) ──────────────────
    ...
    def _handle_api_compose(self):
        ...
        recipient = body.get("recipient", "")   # coordinate or alias of intended recipient
        reply_to  = body.get("reply_to", None)  # message_id being replied to
        reply_id  = body.get("reply_id", None)  # alias/short-id of the reply thread
    ...
    def _handle_publish(self):
        ...
        Body: {
          ...
          "coordinate":  "...",              ← optional BNS coordinate
        }

        Registers a burris:// URL in the published URLs list and optionally
        in LatticeFS URL index if the fs supports it.

        How users publish links:
          1. Host your content at a coordinate in your universe.
          2. POST to /api/publish with the burris:// URL.
          3. Share the burris:// URL with others; they resolve it via their node.
          4. GET /api/publish/list to see all published URLs on this node.
        """
        ctx  = self.daemon_ctx
        body = self._read_json_body()
        if not body:
        ...
        # Register in LatticeFS URL index if supported
    ...
    def _handle_bbs_post(self):
        ...
        Odin's Hall BBS — public dispute room policy:
          - No moderation by host. Users filter their own feed.
          - admin_post rooms require X-OdinNet-Token.
          - open rooms accept any authenticated post.
        """
        ctx  = self.daemon_ctx
        body = self._read_json_body()
        if not body:
    ...
    def _handle_privacy_block(self):
        ...
        body = self._read_json_body()
        if not body:
            self._respond(400, "application/json", b'{"ok":false,"error":"Invalid JSON body"}')
            return
        target = body.get("target", "").strip()
        if not target:
            self._respond(400, "application/json", b'{"ok":false,"error":"target required"}')
            return
        with ctx._blocked_lock:
            data = _load_blocked()
            if target not in data["blocked"]:
                data["blocked"].append(target)
            # Remove from muted if being fully blocked
...
# ─────────────────────────────────────────────────────────────
# OdinWeb server thread  (v6: auto-recover from port conflict)
# ─────────────────────────────────────────────────────────────
...
def _print_startup_banner(
    ...
    # Show actual LAN IP so user knows what to type in browser
    lan_ip = "127.0.0.1"
    try:
    ...
    print(f"  Poll cycle  : every {POLL_INTERVAL_SEC}s  (RT every {POLL_RT_INTERVAL_SEC}s)")
    print(f"  Anomaly Det : every {ANOMALY_INTERVAL_SEC//60}min")
    print(f"  New v6 API  : /api/publish  /api/bbs/*  /api/privacy/*")
    print(f"{border}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="OdinNet Daemon v6 — Burris Numerical System Temporal Node",
