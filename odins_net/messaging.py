...
def pause():
    input("Press Enter to continue...")
            ...
            while True:
                line = input()
                if not line and body_lines:  # stop on empty line after content
                    break
                body_lines.append(line)
            body = "\n".join(body_lines)
            ...
            if sub_choice == "1":
                ...
                if board_pick:
                    try:
                        ...
                        print("Invalid selection – defaulting to Odins-Hall")
            ...
            if msg.delivery_date:
                print(f"  Queued for delivery on: {msg.delivery_date}")
            else:
