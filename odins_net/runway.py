...
class Runway:
    ...
    def __init__(
        ...
        end_mask: Optional[int] = None,          # ← added here to fix the TypeError
        base: int = 10,
        limit: int = 4,
        low: int = 20000,
        high: int = 39999,
        ...
        Args:
            ...
            end_mask: Optional upper/end mask value (often ODINS_HALL_END or high)
        ...
        # End/upper mask – newly added parameter
    ...
    def advance(self, bit: int = 0) -> int:
        ...
        if self.current_mask > self.end_mask:               # ← now uses end_mask
            self.current_mask = self.low
            self.sign = 1
        elif self.current_mask < self.low:
            self.current_mask = self.end_mask               # ← now uses end_mask
            self.sign = -1
...
def create_hall_runway(start_mask: int = 20000, end_mask: Optional[int] = None) -> Runway:
