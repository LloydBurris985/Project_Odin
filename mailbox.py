...
def send_message():
    ...
    mlog(f"SENT fixed 1024B (from {from_date} → to {to_date})")

def poll():
    mlog("POLL - checking proofs + traffic from airport")
    if os.path.exists(MAILBOX_CMD):
        lines = open(MAILBOX_CMD).readlines()
        for line in lines:
            if "proof" in line:
                mlog(f"VERIFIED message sent at coord {line.split()[-1]}")
            ...
            try:
                ...
                if msg.get("to_addr") == MY_ADDR:
                    ...
                    if msg["to_date"] <= now:
                        ...
                        mlog(f"DELIVERED from airport (to_date reached)")
                    else:
                        mlog(f"QUEUED future message (show received for board health)")
            except: pass
