...
v2 patch note (Scotty/Claude, paired with odinnet_daemon.py v12.2.0)
----------------------------------------------------------------------
StatelessCommsNode previously hardcoded PassphraseGeometry(passphrase) with
no way to pass chart_base/mask_base/num_digits/num_n_streams — meaning it
always ran at the 150-digit default, which does NOT match a real, previously
established coordinatefile.json (verified case: num_digits=80). Added
optional constructor params, all defaulting to the prior hardcoded values so
nothing breaks if a node has no coordinatefile.json to parametrize from.
...
# ---------------------------------------------------------------------------
# KDF backend selection (Phase 2B) — Argon2id preferred, scrypt fallback,
# PBKDF2-HMAC-SHA256 as a last resort that's always in the stdlib.
# ---------------------------------------------------------------------------
try:
...
# Mobile-first Argon2id parameters (OWASP's low-memory-safe profile:
# m=19456 KiB / ~19MB, t=2, p=1). NEEDS ON-DEVICE BENCHMARK CONFIRMATION —
# see kdf_benchmark.py. I could not test the real Argon2id path in this
# sandbox (no network access to install argon2-cffi) — only the scrypt and
# PBKDF2 fallback paths are verified here.
ARGON2_TIME_COST = 2
ARGON2_MEMORY_COST_KIB = 19456
ARGON2_PARALLELISM = 1

# Scrypt fallback parameters (textbook "interactive" profile, ~16MB):
# n=2^14, r=8, p=1. Tested in-sandbox — see benchmark output.
SCRYPT_N = 2 ** 14
SCRYPT_R = 8
SCRYPT_P = 1

# Last-resort PBKDF2 fallback iteration count (OWASP 2023 minimum for
# PBKDF2-HMAC-SHA256 is 600,000 — matches what chart_generator.py already
# uses elsewhere in this codebase for consistency).
...
class PassphraseGeometry:
    ...
    v3 (Phase 2B — KDF hardening, council-ratified corrected design)
    -------------------------------------------------------------------
    Previously: passphrase -> random.Random(seed) directly. Deterministic,
    but zero one-wayness — anyone suspecting a passphrase could test it
    against this function with no computational cost gate at all.

    Now: passphrase -> Argon2id (scrypt fallback) -> base secret, evaluated
    ONCE per time-window (default 4h) and cached; base secret ->
    HMAC-SHA256 -> per-window payload seed, replacing the raw passphrase
    as _derive_rand_payload()'s seed.

    CORRECTED FROM THE ORIGINAL 2B RATIFICATION: the initial council design
    included per-node jitter on window rotation and node_id in the KDF
    domain-separation input. Both were removed after implementation review
    surfaced that they break the protocol's core invariant — two peers
    sharing only a passphrase, with no communication, must derive the
    IDENTICAL coordinate. Node-specific jitter or node identity in the KDF
    means peers derive different secrets, permanently or during rotation
    windows. Full council (Claude, ChatGPT, Grok, Gemini, Burris) approved
    this correction without requiring a re-vote — recorded here as the
    permanent engineering clarification:

        "Node-specific jitter and node identity were removed from the KDF
        inputs because they violate the stateless shared coordinate
        derivation invariant. Traffic-analysis mitigations are handled in
        the 2A polling scheduler instead. Windows remain strictly
        time-based and identical across peers."

    Honest limitation, not glossed over: because both peers must derive
    the identical base secret with zero communication, the KDF salt is
    necessarily deterministic (domain tag + window id), not random. This
    is an unavoidable consequence of the stateless design — it means
    Argon2id's usual defense against precomputed rainbow tables is reduced
    to "an attacker needs one table per window id," not eliminated. Still
    a large improvement over the prior zero-cost-gate design, but real
    security engineering means saying so plainly rather than implying
    KDF hardening is a silver bullet here.
    ...
    window_sec   : NEW (v3) — KDF window duration in seconds (default 4h).
                   Every peer computes the same window_id from wall-clock
                   time alone: window_id = int(time.time() // window_sec).
    """

    DOMAIN_TAG = "OdinNet-PassphraseGeometry-v1"
    DEFAULT_WINDOW_SEC = 4 * 60 * 60  # 4 hours, per council vote
    ...
    def __init__(
        ...
        # NOTE (v3): must be RLock, not Lock — state() holds this lock while
        # calling down into _derive_rand_payload() -> _get_base_secret(),
        # which also acquires it. A plain Lock() self-deadlocks here.
        self._lock         = threading.RLock()

        # NEW (v3): base-secret cache, keyed by window_id. Invalidated only
        # on an actual passphrase change (see invalidate_base_secret()) —
        # NOT on fleet-jump/DEFCON events, which are a separate, unrelated
        # concept in OdinNetSecurity and would risk desyncing peers who
        # haven't jumped yet if wired in here.
        self._cached_window_id = None
        self._cached_base_secret = None

    # ── KDF (Phase 2B) ───────────────────────────────────────────────────

    def _current_window_id(self) -> int:
        """Every peer computes this identically from wall-clock time alone."""
        return int(time.time() // self.window_sec)

    def _derive_base_secret(self, window_id: int) -> bytes:
        """
        Passphrase -> Argon2id (scrypt fallback) -> 32-byte base secret.
        Salt is deterministic (domain tag + window id) by necessity — see
        class docstring for why, and the honest limitation that implies.
        """
        kdf_input = f"{self.DOMAIN_TAG}:{self.passphrase}".encode("utf-8")
        salt = hashlib.sha256(f"{self.DOMAIN_TAG}:window:{window_id}".encode("utf-8")).digest()[:16]
        ...
        if _HAVE_ARGON2:
            ...
            # Last-resort fallback if neither is available: PBKDF2-HMAC-SHA256
            # via hashlib, always present in the stdlib. Weaker than Argon2id/
            # scrypt (no memory-hardness) but still a real one-way cost gate —
            # strictly better than the old raw-passphrase-into-random.Random()
            # design, and never silently skipped without a loud log line.
            print("[PassphraseGeometry][WARN] Neither argon2-cffi nor scrypt "
                  "available — falling back to PBKDF2-HMAC-SHA256 (weaker, "
                  "no memory-hardness). Install argon2-cffi for the intended "
                  "mobile-tuned KDF.")
            return hashlib.pbkdf2_hmac("sha256", kdf_input, salt, PBKDF2_FALLBACK_ITERATIONS, dklen=32)

    def _get_base_secret(self) -> bytes:
        """Returns the cached base secret for the current window, deriving on refresh."""
        window_id = self._current_window_id()
        with self._lock:
            if self._cached_window_id != window_id:
                self._cached_base_secret = self._derive_base_secret(window_id)
                self._cached_window_id = window_id
                # Log window transitions for auditability — hash only, NEVER
                # the secret value itself.
                secret_fingerprint = hashlib.sha256(self._cached_base_secret).hexdigest()[:12]
                print(f"[PassphraseGeometry] KDF window refreshed: window_id={window_id} "
                      f"secret_fp={secret_fingerprint}", flush=True)
            return self._cached_base_secret

    def invalidate_base_secret(self):
        """
        Call this when the passphrase actually changes (e.g. OdinCommsEngine
        .register_passphrase() with a new key). Forces the next
        _derive_rand_payload() call to re-run the KDF immediately rather
        than waiting for the window to naturally expire.
        """
        with self._lock:
            self._cached_window_id = None
            self._cached_base_secret = None
        # Also clear the geometry cache, since it was built from the old
        # payload and would otherwise silently serve stale V/R.
        self._state = None
        self._window = None
    ...
    def _derive_rand_payload(self) -> bytes:
        """
        Deterministic bytes seeded by the CURRENT WINDOW's HMAC-derived
        payload seed (was: raw passphrase directly). HMAC runs once per
        window refresh (cheap), not per individual poll — matches how
        state() actually caches and StatelessCommsNode.poll() actually
        re-scans the same fixed window repeatedly, rather than the
        originally-ratified-but-code-mismatched "per poll" description.
        """
        base_secret = self._get_base_secret()
        seed = hmac.new(base_secret, f"{self.DOMAIN_TAG}:payload".encode("utf-8"),
...
class MessageFrame:
    ...
    def build(cls, body: str, to_date: str = None, subject: str = "") -> str:
        ...
        # FIXED: SUBJECT now written BEFORE the header separator, matching
        # what parse() actually reads. Previously SUBJECT was appended after
        # HEADER_SEP, so parse()'s field loop (which stops at the first "---"
        # line) never saw it — subject silently ended up inside payload
        # instead of h["subject"], every single time. Confirmed via the
        # PollingManager integration test: decoded messages showed
        # subject="(no subject)" even when a real subject was sent.
        if subject:
...
class CoordScanner:
    ...
    def scan(self, low: int = None, high: int = None,
        ...
        for i in range(steps):
            ...
            # PATCHED (v3): hm.from_int() does NOT grow hm.D to fit large
            # values — unlike HandMath.add/mul_scalar, which dynamically
            # grow D on carry. probe_V can legitimately need more limbs
            # than the baseline geometry's num_digits (e.g. after a lot of
            # padding), and without this resize, from_int() silently
            # truncates the high-order limbs — a correctness bug, not an
            # error. This mirrors the resize-before-decode pattern already
            # used correctly in FoldingChartGenerator.decode_bytes().
            needed = 0
            v_tmp = probe_V
            while v_tmp > 0:
                needed += 1
            ...
            # PATCHED (v3): previously discarded future-dated messages here
            # via `if not MessageFrame.temporal_filter(header): continue` —
            # that made decode-but-hold impossible, since deferred messages
            # never reached the caller at all. Now every valid header is
            # decoded and returned, tagged with 'due' so the caller (daemon
            # / PollingManager) can route due vs. deferred separately.
            due = MessageFrame.temporal_filter(header)
            ...
            results.append({
                ...
                "raw_text":   text,
                "due":        due,   # NEW (v3) — True = deliverable now, False = embargo
            })
...
class StatelessCommsNode:
    ...
    chart_base    : NEW (v2) — ChartGenerator BASE, default 256
    mask_base     : NEW (v2) — HandMath limb modulus, default 1_000_000_000_000
    num_digits    : NEW (v2) — coordinate width in limbs, default 150
                    (pass 80 to match a real coordinatefile.json identity)
    num_n_streams : NEW (v2) — stream count, default 12
    num_samples   : NEW (v2) — polling-window sample count, default 30
    """
    ...
    def __init__(
        ...
        self._lock            = threading.RLock()
        self._inbox           = []        # list of received (due) message dicts
        self._sent_log        = []        # list of sent coordinate records
        self._deferred_pending = []       # NEW (v3): decoded-but-future-dated hits
                                           # awaiting persistence to LatticeFS Space 5
                                           # by the caller — drained via pop_deferred_pending()
        self._poll_count      = 0
    ...
    def poll(self, steps: int = 100) -> list:
        ...
        new_hits = []          # due hits only — unchanged return contract
        new_deferred = []
        with self._lock:
            for hit in hits:
                pv = hit.get("probe_V", "")
                if pv in self._last_seen_vs:
                    continue
                ...
                if hit.get("due", True):
                    ...
                    self._deferred_pending.append(hit)
                    new_deferred.append(hit)
            ...
            if len(self._deferred_pending) > 500:
                self._deferred_pending = self._deferred_pending[-500:]

            self._poll_count     += 1
            self._last_poll_ts    = MessageFrame.now_str()

        print(f"[StatelessNode] Poll #{self._poll_count}  "
              f"new_due={len(new_hits)}  new_deferred={len(new_deferred)}  "
              f"inbox={len(self._inbox)}", flush=True)
        return new_hits  # unchanged: still returns only due hits, same as before

    def pop_deferred_pending(self) -> list:
        """
        NEW (v3): drains and returns deferred (decoded-but-embargoed) hits
        accumulated since the last drain. The caller (daemon/PollingManager)
        is responsible for persisting these to LatticeFS Space 5 — this
        class stays storage-agnostic and doesn't touch LatticeFS itself.
        """
        with self._lock:
            drained = self._deferred_pending
