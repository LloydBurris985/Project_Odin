...
import json                                 from datetime import datetime, timedelta
                                            def generate_mask_list(seed, length=20000000):                                              """Generate reproducible 20M mask using seed (isoformat string works)."""
    random.seed(seed)
    return [random.randint(0, 255) for _ in range(length)]

def bytes_to_symbols(data_bytes):
    """Pack 8-bit bytes → 6-bit symbols (base-64)."""
    symbols = []
    bit_buffer = 0
    bit_count = 0
    for b in data_bytes:
...
def symbols_to_bytes(symbols, original_length):
    """Unpack 6-bit symbols back to original byte length (truncates padding)."""
...
def decode_lattice(encoded, start_p, seed_str, num_symbols, original_length):
    ...
    for i in range(num_symbols):
        ...
        symbols.append(d)
        p += 1
    # Skip instructions block (we already know length from coord)
    i = num_symbols
    if i < len(encoded) and encoded[i] == 65:
        i += 9
    if i < len(encoded) and encoded[i] != 64:
        print("WARNING: EOF missing — possible timeline drift")
    return symbols_to_bytes(symbols, original_length)

def main():
    parser = argparse.ArgumentParser(description="BNS - Lattice Read/Write")
