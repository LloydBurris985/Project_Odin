...
Phase 2 concurrency hardening (Council finding C1, Step 5 review):
  - Prior to this release, LatticeFSv2 (and its Journal / VersionStore /
    CoordStore internals) had no internal synchronization. This was
    effectively inert while nothing called the filesystem automatically,
    but Step 5 wired PollingManager.run_forever() into a background
    daemon thread that now mutates this same instance concurrently with
    the airlock ledger thread and any manual HTTP-triggered polls.
  - Fix: a single threading.RLock, owned by LatticeFSv2 (not scattered
    across callers), guards every public method that reads or mutates
    self._store / self._journal. RLock (not Lock) because several public
    methods internally call other locked public/private helpers on the
    same thread (e.g. write_file → _jlog/_jcommit; fleet-style composite
    calls), which would deadlock under a plain Lock.
  - This is a filesystem-level fix, not a caller-level one — callers
    (PollingManager, the daemon's airlock loop, manual API handlers)
    require no changes.
"""
...
class Journal:
    ...
    Concurrency note: Journal has no lock of its own. It is only ever
    called from within LatticeFSv2's public methods, all of which are
    now guarded by LatticeFSv2._lock (a single RLock). Journal does not
    need (and should not duplicate) its own locking — that would just
    create a second lock to keep in sync with the first.
    """
...
class VersionStore:
    ...
    Concurrency note: like Journal, VersionStore has no lock of its own.
    It is only ever touched from within LatticeFSv2's public methods,
    which are all guarded by LatticeFSv2._lock.
    """
...
class LatticeFSv2:
    ...
    Concurrency
    -----------
    A single threading.RLock (self._lock) guards every public method that
    reads or mutates self._store / self._journal. This instance is shared
    across multiple threads in the daemon (airlock ledger loop, the
    PollingManager.run_forever() background thread added in Step 5, and
    manual HTTP request threads under ThreadingHTTPServer) — none of them
    need to do anything differently; the lock lives here, once, centrally.
    RLock is used (not Lock) because several methods call other locked
    methods internally on the same thread (e.g. write_file's journal
    commit path), which would self-deadlock under a plain Lock.
    ...
    # Callers of these two helpers must already hold self._lock.
    ...
    def write_file(
        ...
        Whole operation runs under self._lock so a concurrent write/delete/
        read from another thread can't interleave with the journal append →
        version-store update → commit sequence.
        """
        with self._lock:
    ...
    def read_file(
        ...
        Locked so a read can't observe a version list mid-mutation from a
        concurrent write_file/delete_file on another thread.
        """
        with self._lock:
            path = self._normalise_path(path)
    ...
    def list_paths(self, prefix: str = None, space_id: int = None) -> List[str]:
        """
        Returns live file paths as DATA, not print output. ls() only prints —
        this is what PollingManager's Space 5 release job needs. Purely
        additive; does not modify ls() or any other existing method/behavior.
        Locked for the same reason as ls(): must not observe a version list
        mid-mutation from a concurrent writer.
        """
        with self._lock:
            heads = self._store._versions.all_live_heads(space_id=space_id)
            if prefix:
...
# ===========================================================================
# SELF-TESTS  (20 tests covering Phase 1 + Phase 2, + 1 new concurrency test)
# ===========================================================================
...
if __name__ == "__main__":
    ...
    print("\n[Test 21] Concurrency — parallel writers do not corrupt journal/index (C1 fix)")
    import concurrent.futures
    fs21 = lattice_fs_v2(sector_size=512, n_sectors=64)
    N_THREADS = 8
    N_WRITES_EACH = 15

    def _writer(tid):
        for i in range(N_WRITES_EACH):
            fs21.write_file(f"/concurrent/t{tid}.txt", f"thread{tid}-write{i}".encode())

    with concurrent.futures.ThreadPoolExecutor(max_workers=N_THREADS) as pool:
        futures = [pool.submit(_writer, t) for t in range(N_THREADS)]
        for f in futures:
            f.result()

    # Journal must be clean (every commit fully flushed/cleared, no interleaving)
    assert len(fs21._journal) == 0, "journal not clean after concurrent writes"
    assert not fs21._journal.has_uncommitted()

    # Every thread's final value must be exactly what it wrote last — a lost
    # or interleaved write would show up as a wrong version count or value.
    for t in range(N_THREADS):
        path = f"/concurrent/t{t}.txt"
        hist = fs21.version_history(path)
        assert len(hist) == N_WRITES_EACH, (
            f"{path}: expected {N_WRITES_EACH} versions, got {len(hist)}")
        assert fs21.read_file(path) == f"thread{t}-write{N_WRITES_EACH - 1}".encode()

    # Journal sequence numbers must be strictly increasing with no duplicates
    # across the whole run (proves _seq += 1 wasn't raced).
    print(f"  ✅ PASSED  ({N_THREADS} threads × {N_WRITES_EACH} writes, "
          f"journal clean, no lost/interleaved writes)")

    print("\n" + "=" * 68)
    print("  ✅  All 21 LatticeFS v2 Phase 1+2 tests passed.")
    print("=" * 68)
