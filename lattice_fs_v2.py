...
class LatticeFSv2:
    ...
    def list_paths(self, prefix: str = None, space_id: int = None) -> List[str]:
        """
        NEW (additive): returns live file paths as DATA, not print output.
        ls() only prints — there was no public way to get a usable list of
        paths in a space, which PollingManager's Space 5 release job needs.
        Purely additive; does not modify ls() or any other existing
        method/behavior.
        """
        heads = self._store._versions.all_live_heads(space_id=space_id)
        if prefix:
